...
static int get_cJOSN_int(cJSON * spec,
    const char * key_name) {
    ...
    if (res && cJSON_IsNumber(res)) {
        ...
        verbose(VV_INFO, stdout, BLUE "[%s]"
            NONE " No %s info found", __FUNCTION__, key_name);
        return -1;
    }
    return 0;
}
...
static int load_json_file(const char * _DIR, char * json_file_buffer) {
    ...
    verbose(VVV_DEBUG, stdout, BLUE "[%s]"
        NONE " Opening file: %s ", __FUNCTION__, _DIR);
    pFILE = fopen(_DIR, "r");
    if (!pFILE) {
        verbose(V_NORMAL, stderr, BLUE "[%s]"
            NONE " Can not load "
            HW_SPEC_FILE " ", __FUNCTION__);
        return -1;
    }
...
static int load_button_pin(cJSON * spec) {
    ...
    if (!button_spec) {
        verbose(VV_INFO, stdout, BLUE "[%s]"
            NONE " This model has no button pin", __FUNCTION__);
        return 0;
    } else {
        ...
        if (!cJSON_IsNumber(pin) || !cJSON_IsNumber(val)) {
            verbose(V_NORMAL, stderr, BLUE "[%s]"
                NONE " Model has invalide button pin and val", __FUNCTION__);
            return -1;
        }
        ...
        if (pin->valueint > 0)
            ...
            verbose(V_NORMAL, stderr, BLUE "[%s]"
                NONE " Model has invalide gpio_pin number", __FUNCTION__);
            return -1;
        }
        ...
        if (val->valueint == 0 || val->valueint == 1)
            ...
            verbose(V_NORMAL, stderr, BLUE "[%s]"
                NONE " Model has invalide gpio_val number (0,1)", __FUNCTION__);
            return -1;
        }
        return 1;
    }
...
static int load_power_pin(cJSON * spec) {
    ...
    if (!power_spec) {
        verbose(VV_INFO, stdout, BLUE "[%s]"
            NONE " This model has no power pin", __FUNCTION__);
        return 0;
    } else {
        ...
        if (!cJSON_IsNumber(pin) || !cJSON_IsNumber(val)) {
            verbose(V_NORMAL, stderr, BLUE "[%s]"
                NONE " Model has invalide pin and val", __FUNCTION__);
            return -1;
        }
...
static int load_LEDs_spec(cJSON * spec) {
    ...
    if (-1 == number)
        ...
        verbose(V_NORMAL, stderr, BLUE "[%s]"
            NONE " Unvalid LED number (0,255)", __FUNCTION__);
        return -1;
    }
